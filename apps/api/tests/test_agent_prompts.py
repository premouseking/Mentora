...
def test_prompt_dynamic_fragments_order():
    text = build_contextual_fragment(
        context=PromptContext(
            ...
            available_tool_names=("search_evidence",),
        )
    )
...
def test_prompt_legacy_dynamic_context_param():
    text = build_instructions(dynamic_context="兼容旧参数")
    assert "<learning_context>" in text
    assert "兼容旧参数" in text
