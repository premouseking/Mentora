...
```text
Electron Main
  -> Window / File / Upload / Notification / Updater
...
```text
Renderer 提交邮箱/密码
  -> preload auth.login() / auth.register()
  -> IPC
  -> Main Process 调用 Django /auth/login/ 或 /auth/register/
  -> safeStorage 保存 Refresh Token
  -> auth.changed 广播登录态
```

不在 renderer 或 URL 中传递长期令牌。首期不注册自定义 URL 协议（如 `mentora://`），
也不走系统浏览器 OAuth 回调；外部链接仅经 `shell.openExternal` 打开，不参与认证。
开发态可通过 `MENTORA_DEV_AUTH_BYPASS` 跳过登录门禁以便联调 UI。
...
- auth.login / auth.register / 401 单飞刷新与 token 隔离测试；
- 更新状态机测试；
...
2. 未登录用户能在客户端内完成注册/登录，主进程持久化 Refresh Token 且 renderer 无法读取。
