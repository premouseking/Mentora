...
与学习档案相同的按钮逻辑，但计划编辑有特殊行为：
- dirty 时标题后红色 * + Pencil 左边显示 ↩ 撤回
- 编辑中显示 ✕ + ✓（无撤回）
- 如果计划已编辑过（planChanged = true），编辑模式下也会显示 ↩ 撤回

**按钮状态表**：

| planEditing | planChanged | 左侧按钮 | 右侧按钮 | 表格行为 |
|:-----------:|:-----------:|---------|---------|---------|
| false | false | — | 🖉 Pencil | 概览模式（滑轨+画布+详情） |
| false | true | ↩ 撤回 | 🖉 Pencil | 概览模式（滑轨+画布+详情） |
| true | false | ✕ 取消 | ✓ 确认 | 编辑模式（可拖拽、可添加） |
| true | true | ↩ 撤回 | 🖉 Pencil | 锁定模式（只读表格，无拖拽/无添加） |

**行为说明**：
- ✓ 确认/保存 → 标记 `planChanged = true`，表格进入锁定模式
- ✕ 取消 → 退出编辑，恢复原始阶段列表，清除 `planChanged`
- ↩ 撤回 → 退出编辑，清除 `planChanged`，恢复概览模式
- 🖉（锁定模式下）→ 重新进入编辑模式（`planEditing = true`，`planChanged` 保持 `true`）
- 锁定模式下：无拖拽手柄、不支持拖拽、无「+」添加按钮、行无 grab 光标
- 生成新学习方案 → `clearAllDirty()` 退出编辑并清除所有 dirty
...
### 6.5 阶段编辑表格（planEditing = true 时替换概览内容）

点击编辑按钮后，整个概览区域（滑轨 + 画布 + 详情栏）替换为阶段编辑表格：
...
- **左右两栏**：左栏「阶段」显示阶段名称，右栏「简介」显示阶段目标描述
- **拖拽排序**：每行最右侧有 `⋮⋮` 拖拽手柄（`GripVertical` 图标），按住拖动可调整阶段顺序
  - 使用 HTML5 Drag & Drop API
  - 拖拽中当前行半透明（`opacity: 0.45`）
  - 列表实时重排
- **+ 添加阶段**：表格底部满宽虚线「+」按钮
  - 点击 → 上方弹出可选阶段列表（已有阶段不显示）
  - 再次点击 → 关闭弹出层
  - 选中阶段 → 添加到末尾
- **阶段池**（`MOCK_PHASE_POOL`）：6 个可选阶段（知识衔接、习题强化、阶段测评、拓展阅读、思维训练、实战模拟）
- **编辑状态保持**：✓ 保存后 `planEditing = true`，表格持续可见，直到生成新方案或撤回

### 6.6 编辑状态管理

```ts
const [editPhases, setEditPhases] = useState<...>([]);   // 编辑中的阶段列表
const [showAddPhase, setShowAddPhase] = useState(false);  // + 弹出层
const [dragIndex, setDragIndex] = useState<number | null>(null);  // 拖拽中索引
```

- 进入编辑 → `editPhases` 深拷贝自 `MOCK_PLAN.phases`
- ✓ 保存 → `planChanged = true`，`planEditing` 保持 `true`
- ✕ 取消 / ↩ 撤回 → `planEditing = false`，`planChanged = false`
- 生成新方案 → `clearAllDirty()` 同时设置 `planEditing = false`
- 阶段池过滤：`MOCK_PHASE_POOL` 中排除 `editPhases` 已存在的 id
...
| 阶段编辑表格（左右两栏 + 拖拽 + 添加阶段 + 编辑后保持可见） | ✅ 完成 |
| 编辑计划具体内容 | 🔴 待后续设计 |
| 真实 AI 对话接入 | 🔴 待后续 |
| 「确认并生成学习方案」后端逻辑 | 🔴 待后续 |
| 「确认并继续调整」后端逻辑 | 🔴 待后续 |
...
| 文件 | 改动 |
|------|------|
| `apps/web/src/components/PhaseSummary.tsx` | 新增 ~11 个状态/变量（editPhases、showAddPhase、dragIndex、MOCK_PHASE_POOL 等）、~8 个 handler 重写（拖拽排序、添加阶段、按钮逻辑）、阶段编辑表格 UI、按钮状态表逻辑、提示小字渲染、阶段滑轨+方向感知 scrollTrackTo、章节画布 PlanOverviewCanvas、阶段/章节详情栏、底部确认按钮、原生 wheel 事件隔离 |
| `apps/web/src/styles.css` | 新增 ~170 行：阶段编辑表格（表头/行/拖拽手柄/弹出层/添加按钮）、overscroll-behavior、章节画布+SVG 箭头、章节节点、资料列表、列表 reset、撤回按钮样式等 |
| `docs/core-features/learning-plan-redesign-v9.md` | 更新 6.1（编辑按钮状态表）、新增 6.5（阶段编辑表格）、6.6（编辑状态管理）、更新功能状态、文件清单 |
