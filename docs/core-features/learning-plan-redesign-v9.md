...
| **v9** | **交互逻辑重设计**：三区变更追踪 + dirty 状态机 + 底部确认方案按钮 + 阶段导航滑轨 + 计划详情框 |
...
│ [第 1 阶段]  基础入门              │
│ 掌握集合、函数等核心概念与基本方法  │
│ 章节 3 章    任务 12 个    预估 8h │
└─────────────────────────────────────┘
```

- badge：绿色背景标签，显示「第 N 阶段」
- objective：灰色小字阶段目标描述
- 统计行：章节数、任务数、预估时长
- 切换轨道阶段时内容联动更新
...
| 文件 | 改动 |
|------|------|
| `apps/web/src/components/PhaseSummary.tsx` | 新增 ~4 个状态变量、~6 个 handler 重写、提示小字渲染、阶段滑轨+详情框、底部确认按钮、原生 wheel 事件隔离 |
| `apps/web/src/styles.css` | 新增 ~170 行：dirty-star、chat-hint、send-row、undo-btn、plan-detail、confirm-btn、:disabled 样式、滚动条样式 |
